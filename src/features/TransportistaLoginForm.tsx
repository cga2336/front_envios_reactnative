...
export function TransportistaLoginForm({ onSuccess, onRegisterPress, notice }: TransportistaLoginFormProps) {
  ...
  async function onSubmit() {
    ...
    try {
      setSubmitting(true);
      const session = await loginTransportista(email.trim(), password);
      onSuccess(session);
    } catch (error) {
  ...
  return (
    <ScrollView style={styles.card} contentContainerStyle={styles.content}>
      <Text style={styles.title}>Inicio de sesión de transportista</Text>
      <Text style={styles.subtitle}>Ingresa tus credenciales para acceder a tu perfil.</Text>
      ...
      <View style={styles.registerRow}>
        <Text style={styles.registerText}>¿Aún no eres transportista?</Text>
